...
def p_programa_s_p(p):
    'p : sentencia p'
    p[0] = p[1] + '\n'  + p[2] + '\n'
def p_programa_ctl_p(p):
    'p : comentario p'
    p[0] = p[1] + '\n'  + p[2] + '\n'
def p_programa_comentario_p(p):
    'p : control p'
    p[0] = p[1] + '\n'  + p[2] + '\n'
def p_programa_lamda(p):
    'p : '
    p[0]
...
def p_funcion_ret_int_mult(p):
    'func_ret_int : MULTIPLICACIONESCALAR LPAREN vec COMA exp_mat COMA VARIABLE RPAREN'
